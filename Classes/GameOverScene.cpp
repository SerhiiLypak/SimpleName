...
unsigned int score;
unsigned int repl;
Scene* GameOverScene::createScene( unsigned int tempScore,unsigned int replace )
{
    score = tempScore;
	repl = replace;
...
bool GameOverScene::init()
{
    ...
    auto retryItem = MenuItemImage::create( "Buttons/Retry.png", "Buttons/RetryClick.png", CC_CALLBACK_1( GameOverScene::GoToGameScene, this ) );
    retryItem->setPosition( Point( visibleSize.width / 2 + origin.x, visibleSize.height / 4 * 3 ) );
    
    auto mainMenuItem = MenuItemImage::create( "Buttons/Menu.png", "Buttons/MenuClick.png", CC_CALLBACK_1( GameOverScene::GoToMainMenuScene, this ) );
   ...
   auto highScore = def->getIntegerForKey("HIGHSCORE FLAPPY",0);
	...
	def->flush();
	//auto newScore = def->getIntegerForKey("HIGHSCORE FLAPPY");
//	def->flush();
